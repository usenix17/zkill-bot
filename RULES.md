...
Rules tell zkill-bot which killmails to care about and what to do when one matches. They live in `config.yaml` under the `rules:` section. You do not need any programming knowledge to write them.
...
**first-match** is usually what you want — one kill triggers one notification. Use **multi-match** if you want a single kill to potentially trigger several different rules (e.g. a corp-loss rule *and* a high-value rule at the same time).
...
Read this as: *"Value at least 500M ISK, AND (in Jita or Amarr OR involves a capital), AND not an NPC kill."*
